...
COMPOUND TRIGGER 
avg_score score.score%TYPE; 
avg_score_by_credit_num score.score%TYPE; 
    ...
    BEGIN 
        ...
        SELECT (SUM(sc.score * sub.credit_num) / 
        SUM(sub.credit_num)) INTO avg_score_by_credit_num FROM score sc inner join subjects sub ON 
        sc.subjects_id = sub.id WHERE sc.student_id = :NEW.student_id; 
    END AFTER EACH ROW; 
END 
trig_avg_score;
/
